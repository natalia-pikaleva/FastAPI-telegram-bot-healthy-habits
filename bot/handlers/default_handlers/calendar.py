...
def create_habit_request_api(chat_id, date, token):
    """Формирует запрос на API для создания новой привычки и возвращаем
...
def update_habit_date_at_request_api(chat_id, date, token):
    """Формирует запрос на API для изменения даты начала действия привычки и возвращаем
    пользователю созданную привычку в виде инлайн кнопок"""
    # Формируем данные для отправки на FastAPI
    habit_payload = {
        "start_at": date.isoformat(),
...
def calendar_callback(call):
    ...
    if action == "DAY":
        ...
        if saved_action == "create_set_date_at":
            create_habit_request_api(chat_id=chat_id, date=date, token=token)
