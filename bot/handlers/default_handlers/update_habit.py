...
def process_title_update(message):
    ...
    bot.send_message(message.chat.id,
                     f"Сейчас сохранена привычка с id: {user_selected_habit[message.chat.id]["habit_id"]}")
...
def callback_update_repeat_period(call):
    ...
    msg = bot.send_message(user_id, "Введите периодичность (например, daily, weekly):")
    bot.register_next_step_handler(msg, process_repeat_period_update)


def process_repeat_period_update(message):
    with SessionLocal() as db:
        token = get_token_for_user(db, message.chat.id)
    if not token:
        bot.send_message(message.chat.id, "Пожалуйста, авторизуйтесь через /start")
